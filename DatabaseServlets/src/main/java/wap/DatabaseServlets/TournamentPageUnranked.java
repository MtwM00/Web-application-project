...
/**
 * Servlet implementation class TournamentPageUnranked
 */
@WebServlet("/TournamentPageUnranked")
public class TournamentPageUnranked extends HttpServlet {
		...
		try {
           ...
           resultString = getHtmlTableFromResultSet(rs);
			...
			if(PairingDAO.checkIfMatchAlreadyUpdated(cookie)) {
	            out.println("<a href='../DatabaseServlets/ShowSummaryUnranked'>Check summary for game</a>");
	            Cookie ck = new Cookie("tournamentId", cookieString);
	        	response.addCookie(ck);
	            }
		} catch (Exception ex) {
